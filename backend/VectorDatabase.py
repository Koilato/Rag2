...
# 可单独使用，直接连接到chromadb
def connect_to_chromadb(chroma_config: dict | None = None) -> chromadb.Client:
    """
    Initializes and returns a persistent ChromaDB client.

    Args:
        chroma_config (dict | None): A dictionary with ChromaDB settings.
                                     If None, loads from 'config.ini'.
    ...
    persistent_path = chroma_config.get('persistent_path', './ChromaDatabase')
...
#     # --- 步骤 5: 执行批量添加操作 ---
#     if not chroma_ids:
#         print("警告: 没有有效数据可以添加到 Chroma DB。流程结束。")
#         return collection
#
#     print(f"数据准备完成。开始将 {len(chroma_ids)} 条记录批量添加到 Chroma DB 集合 '{collection_name}'...")
#     try:
#         # 这是函数的核心写入操作
#         collection.add(
...
#     except Exception as e:
#         print(f"错误: 将数据添加到 Chroma DB 时发生严重错误。流程终止。错误信息: {e}")
...
def get_full_documents_from_mysql(uuids_from_chroma: list, db_name: str = "rag"):
    ...
    此函数会解码UUID以确定表名（格式为 YYYY-MM-DD），并聚合查询。
